...
@app.route('/predict',methods=['POST', "GET"])# gets the values that were sent to '/predict' by 'index.html'
def predict():
    ...
    return render_template('index.html', prediction_text='Iris species: {}'.format(output), data=int_features)# displays the prediction inside the '<b>{{ prediction_text }}</b>' that we've seen in 'index.html'
